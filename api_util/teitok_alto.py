    ...
    try:
        ...
        for page_idx, page in enumerate(root.iter(_tag('Page')), start=1):
            ...
            alto_pages.append({
                ...
                'width': page_w_str, 'height': page_h_str, 'idx': page_idx,
                'ps_hpos': ps_hpos, 'ps_vpos': ps_vpos,
                ...
                for line in block.iter(_tag('TextLine')):
                    ...
                    for string in line.iter(_tag('String')):
                        ...
                        try:
                            ...
                            alto_strings.append({
                                'content': content,
                                'left': int(hpos), 'top': int(vpos),
                                'right': int(hpos + width), 'bottom': int(vpos + height),
                                'page_idx': page_idx, 'block_id': block_id,
                                'line_id': line_id, 'line_bbox': line_bbox,
                            })
                        except (ValueError, TypeError):
                            pass
                ...
                for graphic in page.iter(_tag(gtag)):
                    try:
                        ...
                        alto_graphics.append({
                            'type': gtag, 'id': graphic.get('ID', ''),
                            'bbox': (int(hpos), int(vpos), int(hpos + width), int(vpos + height)),
                            'page_idx': page_idx,
                        })
                    except (ValueError, TypeError):
                        pass
...
def _align_tokens_to_alto(tokens, alto_strings):
    ...
    for t_idx in range(len(tokens)):
        ...
        if len(page_indices) > 1:
            ...
            print(f"  [Warn] Token '{form}' spans pages {sorted(page_indices)}; "
                  "using first matched page for bbox assignment.", file=sys.stderr)
        bboxes[t_idx] = {
                ...
                if not line.strip() or line.startswith('#'):
                    if not line.strip() and current_tok:
                        sentences.append({
                            ...
                            'tokens': current_tok, 'page_break': pending_page_break,
                        })
                        current_tok = []
                        pending_page_break = False
                    continue
