...
def _read_csv(file_path):
    ...
    try:
        with open(file_path, "r", encoding="utf-8") as fh:
            ...
            for row in reader:
                try: p = int(row.get("page_num", 0) or 0)
                except (ValueError, TypeError): p = 0
                try: ln = int(row.get("line_num", 0) or 0)
                except (ValueError, TypeError): ln = 0
                text = (row.get("text") or "").strip()
                if text: entries.append({"p": p, "l": ln, "text": text})
    except Exception as exc: print(f"[Error] reading CSV {file_path}: {exc}", file=sys.stderr)
    return entries
...
def _read_xlsx(file_path):
    ...
    try:
        ...
        for sheet_name in wb.sheetnames:
            ...
            if not headers or "text" not in headers: continue
            ...
            for row in ws.iter_rows(min_row=2, values_only=True):
                ...
                try: p = int(row[page_idx]) if page_idx != -1 and row[page_idx] is not None else 0
                except (ValueError, TypeError): p = 0
                try: ln = int(row[line_idx]) if line_idx != -1 and row[line_idx] is not None else 0
                except (ValueError, TypeError): ln = 0
                entries.append({"p": p, "l": ln, "text": text})
    except Exception as exc: print(f"[Error] reading XLSX {file_path}: {exc}", file=sys.stderr)
    return entries

def get_sorted_text_and_page_count(file_path):
    ext = os.path.splitext(file_path)[1].lower()
    if ext == ".csv": entries = _read_csv(file_path)
    elif ext == ".xlsx": entries = _read_xlsx(file_path)
    else: return None, 0
