...
def process_teitok_file(file_path, dx, dy, sx, sy):
    ...
    if mutated:
        ...
        with open(file_path, 'w', encoding='utf-8') as f:
            f.write(out_str)
        print(f"  [Success] Patched bounding box offsets.")
    else:
...
def main():
    ...
    parser.add_argument("-i", "--input", required=True, help="Path to TEITOK XML file or directory of files.")
    parser.add_argument("--dx", type=float, default=0.0, help="Horizontal shift adjustment (e.g. -297 to remove left margin).")
