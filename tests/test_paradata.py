...
class TestParadataLoggerLifecycle:
    ...
    def test_filename_contains_run_id_and_program(self, tmp_path):
        logger = ParadataLogger(PROGRAM_NAME, {}, paradata_dir=str(tmp_path))
        path = logger.finalize()
        stem = Path(path).stem  # e.g. "260315-120442_nlp-enrich"
...
class TestParadataLoggerCounters:
    ...
    def test_log_document_success_takes_precedence_over_output_counts(self, tmp_path):
        """
        When log_document_success() is called at least once, finalize() must
        report its counter as successfully_processed rather than falling back
        to max(output_counts).  This matters for pipelines like api_3_nt that
        record N page-level TSV files per document.
        """
        logger = ParadataLogger(PROGRAM_NAME, {}, output_types=["tsv"],
                                paradata_dir=str(tmp_path))
        logger.log_success("tsv", count=20)  # 20 page TSVs…
        logger.log_document_success()         # …but only 1 input document
        path = logger.finalize()
        data = json.loads(Path(path).read_text())
        assert data["statistics"]["successfully_processed"] == 1
...
class TestParadataLoggerConfigSnapshot:
    ...
    def test_state_roundtrip_preserves_counts_and_skips(self, tmp_path):
        """_to_state_dict / _from_state_dict round-trip must be lossless."""
        logger = ParadataLogger(PROGRAM_NAME, {"a": 1},
                                paradata_dir=str(tmp_path))
        logger.log_success("csv", 5)
        logger.log_success("xml", 2)
        logger.log_skip("f.xml", "parse error")
        state = logger._to_state_dict()
        restored = ParadataLogger._from_state_dict(state)
        assert restored._output_counts == {"csv": 5, "xml": 2}
        assert len(restored._skipped) == 1
        assert restored._skipped[0]["file"] == "f.xml"
        assert restored.program == PROGRAM_NAME